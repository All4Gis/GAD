...
from PyQt5.QtWidgets import QTextEdit,QWidget,QApplication, QSizePolicy, QLabel
...
from .qad_entity import QadEntity, QadEntityGeomTypeEnum
...
#    """
   ...
   def reset(self):
      ...
      self.nputMode = QadInputModeEnum.NONE
   ...
   def setLinesMarker(self, points):
      ...
      lineMarker = createRubberBand(self.canvas, QgsWkbTypes.LineString, True)
...
#    """
   ...
   def __init__(self, plugIn):
      ...
      self.prevPart = None # parte precedente il punto da spostare in modo grip (QadLinearObject)
      self.nextPart = None # parte successiva il punto da spostare in modo grip (QadLinearObject)
   ...
   def setPrevPart(self, linearObject):
      if linearObject is None:
         ...
         if self.prevPart is None:
            self.prevPart = qad_utils.QadLinearObject(linearObject)
         else:
   ...
   def setNextPart(self, linearObject):
      if linearObject is None:
         ...
         if self.nextPart is None:
            self.nextPart = qad_utils.QadLinearObject(linearObject)
         else:
   ...
   def setPrevNextPart(self, entity, gripPoint):
      ...
      if entity.isDimensionComponent():
         ...
         if entityType == QadEntityGeomTypeEnum.ARC:
            arc = entity.getQadGeom(gripPoint.atGeom, gripPoint.atSubGeom) # arco in map coordinate
            if gripPoint.gripType == QadGripPointTypeEnum.ARC_MID_POINT:
               prevPart = qad_utils.QadLinearObject()
               prevPart.setArc(arc, False)
               nextPart = qad_utils.QadLinearObject()
               nextPart.setArc(arc, False)
            else:
               if qad_utils.ptNear(arc.getStartPt(), gripPoint.getPoint()):
                  nextPart = qad_utils.QadLinearObject()
                  nextPart.setArc(arc, False)
               elif qad_utils.ptNear(arc.getEndPt(), gripPoint.getPoint()):
                  prevPart = qad_utils.QadLinearObject()
                  prevPart.setArc(arc, False)
                  
         elif entityType == QadEntityGeomTypeEnum.LINESTRING:         
            linearObjectList = entity.getQadGeom(gripPoint.atGeom, gripPoint.atSubGeom) # linestring in map coordinate
            prevPart, nextPart = linearObjectList.getPrevNextLinearObjectsAtVertex(gripPoint.nVertex)
            if (gripPoint.gripType == QadGripPointTypeEnum.LINE_MID_POINT or \
               ...
               prevPart = qad_utils.QadLinearObject(nextPart)
            
         elif entityType == QadEntityGeomTypeEnum.CIRCLE:         
            circle = entity.getQadGeom(gripPoint.atGeom, gripPoint.atSubGeom)# cerchio in map coordinate
            if circle.isPtOnCircle(gripPoint.getPoint()):
               prevPart = qad_utils.QadLinearObject()
               prevPart.setSegment(circle.center, gripPoint.getPoint())
   ...
   def mouseMoveEvent(self, mousePos):
      ...
      if self.prevPart is not None:
         if self.prevPart.isSegment():
            ...
            arc = self.prevPart.getArc()
            if self.edits[QadDynamicInputEditEnum.EDIT_DIST_PREV_PT].isVisible():
               # usato per lunghezza raggio arco
               self.edits[QadDynamicInputEditEnum.EDIT_DIST_PREV_PT].showMsg(qad_utils.numToStringFmt(arc.radius), False, False, False) # senza aggiornare la posizione dei controlli
               
            if self.edits[QadDynamicInputEditEnum.EDIT_REL_DIST_PREV_PT].isVisible():
               # usato per lunghezza raggio arco
               self.edits[QadDynamicInputEditEnum.EDIT_REL_DIST_PREV_PT].showMsg(qad_utils.numToStringFmt(arc.radius), False, False, False) # senza aggiornare la posizione dei controlli
            ...
            if self.edits[QadDynamicInputEditEnum.EDIT_ANG_PREV_PT].isVisible():
               ...
               if self.prevPart.getEndPt() == arc.getEndPt():
                  angle = arc.endAngle
               else:
      ...
      if self.nextPart is not None:
         if self.nextPart.isSegment():
            ...
            arc = self.nextPart.getArc()
            if self.edits[QadDynamicInputEditEnum.EDIT_DIST_NEXT_PT].isVisible():
               # usato per lunghezza raggio arco
               self.edits[QadDynamicInputEditEnum.EDIT_DIST_NEXT_PT].showMsg(qad_utils.numToStringFmt(arc.radius), False, False, False) # senza aggiornare la posizione dei controlli
               
            if self.edits[QadDynamicInputEditEnum.EDIT_REL_DIST_NEXT_PT].isVisible():
               # usato per lunghezza raggio arco
               self.edits[QadDynamicInputEditEnum.EDIT_REL_DIST_NEXT_PT].showMsg(qad_utils.numToStringFmt(arc.radius), False, False, False) # senza aggiornare la posizione dei controlli
               
            if self.edits[QadDynamicInputEditEnum.EDIT_ANG_NEXT_PT].isVisible():
               if self.nextPart.getStartPt() == arc.getStartPt():
                  angle = arc.startAngle
               else:
                  angle = arc.endAngle
               if angle >= math.pi and angle < 2 * math.pi:
            ...
            if self.edits[QadDynamicInputEditEnum.EDIT_REL_ANG_PREV_PT].isVisible():
               ...
               msg = qad_utils.numToStringFmt(qad_utils.toDegrees(arc.totalAngle())) + u'\N{DEGREE SIGN}' # simbolo dei gradi
      ...
      if self.prevPart is not None:
         ...
         if self.prevPart.isSegment():
            ...
            if self.edits[QadDynamicInputEditEnum.EDIT_ANG_PREV_PT].isVisible():
               # usato per angolo arco nel punto finale della parte precedente
               p2 = self.prevPart.getEndPt()
               p1 = QgsPointXY(center.x() + arc.radius, center.y())
            ...
            if self.edits[QadDynamicInputEditEnum.EDIT_REL_DIST_PREV_PT].isVisible():
               ...
               p1 = self.prevPart.getArc().center
            ...
            if self.edits[QadDynamicInputEditEnum.EDIT_DIST_PREV_PT].isVisible():
               ...
               if self.nextPart is not None and self.nextPart == self.prevPart:
                  p2 = self.prevPart.getArc().getMiddlePt()
               else:
                  p2 = self.prevPart.getEndPt()
               offset = 0
            else:
      ...
      if self.nextPart is not None:
         ...
         if self.nextPart.isSegment():
            ...
            if self.edits[QadDynamicInputEditEnum.EDIT_REL_DIST_NEXT_PT].isVisible():
               ...
               p1 = self.nextPart.getArc().center
               p2 = self.nextPart.getEndPt()
               editPt, lineMarkers = self.getPosAndLineMarkerForLine(p1, p2, \
         ...
         # se devo mostrare i widget delle quote
            ...
            if self.prevPoint is not None: # si tratta di inserimento di un nuovo punto a fine linea
               ...
               if self.dynDiVis == 0 or self.dynDiVis == 1: # solo una quota alla volta (0) o due quota alla volta (1)
                  if self.prevPart is not None and self.prevPart.isSegment():
                     return QadDynamicInputEditEnum.EDIT_DIST_PREV_PT
                  elif self.nextPart is not None and self.nextPart.isSegment():
         ...
         if (self.isDimensionalWidgetVisib() == False and self.dynDiVis == 2) or \
            ...
            if self.isDimensionalWidgetVisib() and self.dynDiVis != 2:
               ...
               if self.prevPoint is not None:
                  ...
                  if (self.prevPart is not None and self.prevPart.isSegment() and \
                     ...
                     (self.nextPart is not None and self.nextPart.isSegment() and \
         ...
         if self.prevPoint is not None: # si tratta di inserimento di un nuovo punto a fine linea
            ...
            if self.dynDiVis == 0: # solo una quota alla volta
               ...
               if self.dynDiGrip & 1: # quota risultante (distanza dal punto precedente)
                  if self.prevPart is not None and self.prevPart.isSegment():
                     visibList[QadDynamicInputEditEnum.EDIT_DIST_PREV_PT] = True
                  if self.nextPart is not None and self.nextPart.isSegment():
...
#                if self.prevPart is not None and self.prevPart.isArc():
...
#                   visibList[QadDynamicInputEditEnum.EDIT_REL_DIST_PREV_PT] = True
#                if self.nextPart is not None and self.nextPart.isArc():
#                   # se nextPart e prevPart sono uguali 
#                   if self.prevPart is not None and self.nextPart == self.prevPart:
#                      # usato per lunghezza raggio nel punto iniziale della parte precedente
   ...
   def mouseMoveEvent(self, mousePos):
      ...
      if self.edits[QadDynamicInputEditEnum.EDIT_DIST_PREV_PT].isVisible() and \
         ...
         if self.prevPoint is not None: # si tratta di inserimento di un nuovo punto a fine linea
            ...
            if self.prevPart.isSegment(): # spostamento di un punto di un segmento in modalità grip
               dist = qad_utils.getDistance(self.prevPart.getStartPt(), self.resPt)
               self.edits[QadDynamicInputEditEnum.EDIT_DIST_PREV_PT].showMsg(qad_utils.numToStringFmt(dist), False, False, False) # senza aggiornare la posizione dei controlli
            elif self.prevPart.isArc(): # spostamento di un punto di un arco in modalità grip
               # usato per lunghezza raggio nel punto finale della parte precedente
               # oppure lunghezza raggio nel punto medio se parte precedente e successiva sono lo stesso arco
               self.edits[QadDynamicInputEditEnum.EDIT_DIST_PREV_PT].showMsg(qad_utils.numToStringFmt(self.prevPart.getArc().radius), False, False, False) # senza aggiornare la posizione dei controlli
      ...
      if self.edits[QadDynamicInputEditEnum.EDIT_ANG_PREV_PT].isVisible() and \
         ...
         elif self.prevPart is not None and self.prevPart.isSegment(): # spostamento di un punto di un segmento in modalità grip
      ...
      if self.edits[QadDynamicInputEditEnum.EDIT_REL_DIST_PREV_PT].isVisible() and \
         ...
         if self.prevPart is not None:
            if self.prevPart.isSegment(): # spostamento di un punto di un segmento in modalità grip
               ...
               # usato per lunghezza raggio nel punto iniziale della parte precedente se arco
               self.edits[QadDynamicInputEditEnum.EDIT_REL_DIST_PREV_PT].showMsg(qad_utils.numToStringFmt(self.prevPart.getArc().radius), False, False, False) # senza aggiornare la posizione dei controlli
      ...
      if self.edits[QadDynamicInputEditEnum.EDIT_REL_ANG_PREV_PT].isVisible() and \
         ...
         if self.prevPart is not None and self.prevPart.isSegment(): # spostamento di un punto di un segmento in modalità grip
      ...
      if self.edits[QadDynamicInputEditEnum.EDIT_DIST_NEXT_PT].isVisible() and \
         ...
         if self.nextPart is not None:
            if self.nextPart.isSegment(): # spostamento di un punto di un segmento in modalità grip
            ...
            elif self.nextPart.isArc(): # spostamento di un punto di un arco in modalità grip
               # usato per lunghezza raggio nel punto iniziale della parte successiva se arco
               self.edits[QadDynamicInputEditEnum.EDIT_DIST_NEXT_PT].showMsg(qad_utils.numToStringFmt(self.nextPart.getArc().radius), False, False, False) # senza aggiornare la posizione dei controlli
      ...
      if self.edits[QadDynamicInputEditEnum.EDIT_REL_DIST_NEXT_PT].isVisible() and \
         ...
         if self.nextPart is not None:
            if self.nextPart.isSegment(): # spostamento di un punto di un segmento in modalità grip
               ...
               # usato per lunghezza raggio nel punto finale della parte successiva se arco
               self.edits[QadDynamicInputEditEnum.EDIT_REL_DIST_NEXT_PT].showMsg(qad_utils.numToStringFmt(self.nextPart.getArc().radius), False, False, False) # senza aggiornare la posizione dei controlli
      ...
      if self.edits[QadDynamicInputEditEnum.EDIT_ANG_NEXT_PT].isVisible() and \
         ...
         if self.nextPart is not None and self.nextPart.isSegment(): # spostamento di un punto di un segmento in modalità grip
         ...
         if self.isDimensionalWidgetVisib():
            if self.prevPoint is not None: # si tratta di inserimento di un nuovo punto a fine linea
               ...
               if self.edits[QadDynamicInputEditEnum.EDIT_DIST_PREV_PT].isVisible() and \
                  ...
                  self.prevPart is not None and self.prevPart.isSegment():
                  dist = self.edits[QadDynamicInputEditEnum.EDIT_DIST_PREV_PT].checkValid() # ritorna il valore se valido
                  if dist is not None:
                     pt1 = self.prevPart.getStartPt()
                     pt2 = self.prevPart.getEndPt()
               ...
               # se il widget delle quote nel modo grip "distanza dal punto successivo" è visibile
                    ...
                    self.nextPart is not None and self.nextPart.isSegment():
         ...
         if self.edits[QadDynamicInputEditEnum.EDIT].isVisible():
            if self.edits[QadDynamicInputEditEnum.EDIT].isLockedValue() == True:
               ...
               if self.resValue is None:
                  ...
                  return True
            elif self.inputType & QadInputTypeEnum.FLOAT and self.prevPoint is not None:
